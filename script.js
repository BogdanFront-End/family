...
function initBurgerMenu() {
    ...
    function openBurgerMenu() {
        console.log('Opening burger menu - top to bottom animation');
        ...
        // Даем время для применения display: block, затем добавляем active
        setTimeout(() => {
    ...
    function closeBurgerMenu() {
        console.log('Closing burger menu - bottom to top animation');
        
        // Убираем активные классы
        burgerMenu.classList.remove('active');
        burgerOverlay.classList.remove('active');
        
        // Восстанавливаем скролл body после анимации
        setTimeout(() => {
    ...
    if (burgerLinks) {
        burgerLinks.forEach(link => {
            link.addEventListener('click', function(e) {
                // Если это не якорная ссылка, закрываем меню
...
function initHorizontalSliders() {
    ...
    sliders.forEach(slider => {
        ...
        slider.addEventListener('mousemove', (e) => {
            ...
            const walk = (x - startX) * 2.5; // Скорость перетаскивания
...
function addScrollIndicators(slider) {
    const checkScroll = () => {
        ...
        if (canScroll && !slider.hasAttribute('data-scrollable')) {
            ...
            slider.removeAttribute('data-scrollable');
            slider.style.cursor = 'default';
        }
    };
